...
function RainyDay(canvasid, sourceid, width, height, opacity, blur) {
    ...
    this.img =  typeof(sourceid) == 'string' ? document.getElementById(sourceid) : sourceid;
...
/* 锟斤拷锟斤拷锟斤拷锟?锟斤拷锟斤拷之锟斤拷 www.lanrenzhijia.com */
/**
...
(function (window) {
    ...
    function initWindow(_outBox) {
        ...
        // 获取相关属性
        var imageSrc = outBox.getAttribute('data-image-src');
        var rect = outBox.getBoundingClientRect();
        outPos.x = rect.left;
        outPos.y = rect.top;
    ...
    function bindEvents() {
        ...
        outBox.addEventListener(clickEvtName, function (e) {
            ...
            var x = (device ? e.touches[0].clientX : e.clientX) - outPos.x + docEle.scrollLeft - docEle.clientLeft;
            var y = (device ? e.touches[0].clientY : e.clientY) - outPos.y + docEle.scrollTop - docEle.clientTop;
            drawPoint(x, y);
    ...
    function drawPoint(x, y) {
        ...
        if (typeof(drawPercentCallback) == 'function') {
            drawPercentCallback.call(null, getTransparentPercent(context, this.width, this.height));
        }
    }
